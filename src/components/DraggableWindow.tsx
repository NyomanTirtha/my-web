...
export const DraggableWindow = ({ title, children, isOpen, onClose, initialPosition = { x: 0, y: 0 }, isMuted = false, windowClassName = 'w-[800px]' }: WindowProps) => {
    const handleClose = () => {
        if (!isMuted) {
        ...
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
                    ...
                    >
                        {/* Window Header */}
                        <div className="h-9 bg-neutral-800/50 border-b border-neutral-700 flex items-center justify-between px-3">
                        ...
                        {/* Content */}
                        <div className="p-6 text-neutral-300 text-sm leading-relaxed overflow-y-auto max-h-[80vh]">
                            {children}
                        </div>
                    </motion.div>
                </div>
