...
function getAssetUrl(image: string, applicationId?: string): string {
    ...
    // Discord media proxy (e.g. external Steam images)
    if (image.startsWith('mp:external/'))
        return `https://media.discordapp.net/external/${image.replace('mp:external/', '')}`;
    // New Discord media format (numeric snowflake IDs without app prefix)
    if (/^\d+$/.test(image) && applicationId)
        return `https://cdn.discordapp.com/app-assets/${applicationId}/${image}.webp`;
    // Classic app-assets (try webp first, fallback handled by img onError)
    if (applicationId)
        return `https://cdn.discordapp.com/app-assets/${applicationId}/${image}.webp`;
    return image;
}
        ...
        <div className="flex items-start gap-3">
            <div className="relative flex-shrink-0">
                {largeImg ? (
                    ...
                    /* Fallback: generic game icon for games without assets (e.g. CS2) */
